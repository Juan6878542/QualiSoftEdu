...
def evaluar():
    ...
    if request.method == 'POST':
        try:
            ...
            normas = float(request.form.get('normas', 0))
            modelos = float(request.form.get('modelos', 0))
            estandares = float(request.form.get('estandares', 0))
            codigo = float(request.form.get('codigo', 0))
            pruebas = float(request.form.get('pruebas', 0))
        except Exception as e:
            flash('Valores inválidos: ' + str(e), 'error')
            return redirect(url_for('evaluar'))

        for v,name in [(normas,'normas'),(modelos,'modelos'),(estandares,'estandares'),
                       (codigo,'codigo'),(pruebas,'pruebas')]:
            if v < 0 or v > 100:
                flash(f'El campo {name} debe estar entre 0 y 100.', 'error')
                return redirect(url_for('evaluar'))

        weights = {'normas':0.15,'modelos':0.15,'estandares':0.15,'codigo':0.25,'pruebas':0.30}
        total_percent = normas*weights['normas'] + modelos*weights['modelos'] + estandares*weights['estandares'] + codigo*weights['codigo'] + pruebas*weights['pruebas']
        score_0_5 = round((total_percent/100)*5, 2)
        ...
        cur = conn.cursor()
        cur.execute('''
            INSERT INTO evaluaciones (id_aplicativo, normas, modelos, estandares, codigo, pruebas,
                                      total_percent, score_0_5, evaluador, fecha_eval, comentarios)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (id_app, normas, modelos, estandares, codigo, pruebas, total_percent, score_0_5, evaluador, datetime.now().isoformat(), comentarios))
...
def historico_export():
    conn = get_db_conn()
    rows = conn.execute('SELECT e.id_evaluacion, a.nombre as aplicativo, e.normas, e.modelos, e.estandares, e.codigo, e.pruebas, e.total_percent, e.score_0_5, e.evaluador, e.fecha_eval FROM evaluaciones e JOIN aplicativos a ON a.id_aplicativo=e.id_aplicativo ORDER BY e.fecha_eval DESC').fetchall()
    conn.close()
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(['id','aplicativo','normas','modelos','estandares','codigo','pruebas','total_percent','score_0_5','evaluador','fecha_eval'])
    for r in rows:
        writer.writerow([r['id_evaluacion'], r['aplicativo'], r['normas'], r['modelos'], r['estandares'], r['codigo'], r['pruebas'], r['total_percent'], r['score_0_5'], r['evaluador'], r['fecha_eval']])
    output.seek(0)
